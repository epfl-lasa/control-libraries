...
namespace state_representation
{
	...
	const DualQuaternionState exp(const DualQuaternionState& state)
	{
		...
		{
			Eigen::Array4d coeffs = (sin(state.get_primary().norm()) / state.get_primary().norm()) * state.get_primary().coeffs();
	...
	{ 
		...
		{
			os << state.get_name() << " DualQuaternionState expressed in " << state.get_reference_frame() << " frame" << std::endl;
